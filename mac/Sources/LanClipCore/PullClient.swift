...
public final class PullClient {
    ...
    /// задачи 26): на Windows-стороне сосед видел систематические ~20% случаев, когда
    /// `seq` буфера соседа менялся между запросом манифеста и запросом блоба — то есть
    /// это не редкая случайность, а фоновый шум (например, служба истории буфера
    /// обмена), с которым сталкивается каждый пятый `pull`. Раньше это долетало до
    /// пользователя как явная ошибка при каждом таком совпадении; теперь протокол сам
