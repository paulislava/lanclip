...
namespace LanClip
{
    ...
    {
        // Держатель хоткея на всё время жизни процесса.
        //
        // Без него ловится очень неприятный баг: `Hotkey` лежал в локальной
        // переменной `Main`, после чего управление уходило в `Application.Run()`.
        // Сборщик мусора .NET вправе счесть локальную переменную мёртвой сразу
        // после последнего обращения к ней — метод при этом ещё не вернулся, — а у
        // `NativeWindow` есть финализатор, уничтожающий HWND. Окно умирает,
        // `RegisterHotKey` снимается, и Ctrl+Shift+V молча перестаёт работать при
        // живом процессе и работающем HTTP.
        //
        // Проверено на живой машине: сразу после перезапуска агента проба
        // `RegisterHotKey(Ctrl+Shift+V)` получала 1409 (занято нами), а у агента,
        // проработавшего часы, — успех, то есть регистрации уже не было.
        //
        // Swift-сторона этим не болеет: там ссылку держит локальная константа
        // функции, которая никогда не возвращается, и ARC её не освобождает.
        // Переносить это рассуждение на .NET нельзя — здесь решает достижимость,
        // а не область видимости.
        static Hotkey hotkeyHolder;
        ...
        static int RunServe(string configPath)
        {
            ...
            hotkeyHolder = new Hotkey(delegate
            {
