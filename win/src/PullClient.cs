...
namespace LanClip
{
    ...
    class PullClient
    {
        ...
        // на этой стороне сосед видел систематические ~20% случаев, когда seq буфера
        // соседа менялся между запросом манифеста и запросом блоба — это не редкая
        // случайность, а фоновый шум (например, служба истории буфера обмена
        // Windows), с которым сталкивается каждый пятый Pull(). Раньше это долетало
        // до пользователя как явная ошибка при каждом таком совпадении; теперь
        // протокол сам пробует ещё раз, прежде чем сдаться.
        //
